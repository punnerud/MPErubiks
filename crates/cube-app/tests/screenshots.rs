...
fn solve_guide_done_confetti() {
    ...
    let solution = cube_solver::solve(&state).expect("table");
    {
        ...
        app.screen = Screen::Solve(cube_app::screens::solve::SolveScreen::Guide(
            {
                let mut g = cube_app::screens::solve::GuideState::plain(solution, state);
                g.cursor = g.solution.0.len();
                g
            },
        ));
    }
    // Confetti repaints continuously - run a fixed number of steps.
    h.run_steps(6);
    h.snapshot("solve_guide_done");
}
